...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # configure window
        imagePath = basedir + '\\assets\\frame0\\app_icon.png'
        iconIm = tkinter.PhotoImage(file = imagePath)
        ...
        self.sidebar_generate_chroma_button = customtkinter.CTkButton(self.sidebar_menu_frame, command=self.generate_chroma, text=getlang('generate_chroma_button'))
        self.sidebar_generate_chroma_button.grid(row=1, column=0, padx=10, pady=5)
    ...
    def open_file(self):
        ...
        if file_path is None:
            return
        self.currentfile = file_path.name
        self.debugger.log(f'File selected: {self.currentfile}')
    ...
    # Method to generate chroma from the selected file.
    def generate_chroma(self):
        if self.currentfile is None:
            self.debugger.warn('No file selected to generate chroma from.')
            return
        self.sidebar_generate_chroma_button.configure(state='disabled')
        chroma_thread = threading.Thread(target = self.process_chroma)
        chroma_thread.daemon = True
        chroma_thread.start()

    def process_chroma(self):
        self.debugger.log('Generating chroma...')
        chromagen = ChromaGenerator()
        chromagen.generate_chroma(self.currentfile)
        chromagen.save_chroma(chromagen.chroma_cq, self.currentfile)
        self.GENERATED = True
        self.debugger.log('Chroma generation complete.')
        self.sidebar_generate_chroma_button.configure(state='normal')
        return

    # Method to change the appearance mode of the UI.
    def change_appearance_mode_event(self, new_appearance_mode: str):
        customtkinter.set_appearance_mode(new_appearance_mode)

    # Method to change the scaling of the UI.
    def change_scaling_event(self, new_scaling: str):
        new_scaling_float = int(new_scaling.replace("%", "")) / 100
        customtkinter.set_widget_scaling(new_scaling_float)

    # Method overrides the default window close behaviour to allow for a clean exit without tkinter errors.
    def on_closing(self):
        self.debugger.log('Closing the application.')
        self.quit()
